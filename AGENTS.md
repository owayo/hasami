...
| IPAdic | 全角の「％」だけが 名詞,接尾,助数詞 で、半角の `%`・`‰` `℃` `°`・CJK 互換文字の単位（`㎏` `㎞` `㌢` `㍍`）は辞書に無い | 未知の記号（記号,一般）になり、数と単位をまとめる処理で句読点と同じ記号になる（Issue #7） | `scripts/prepare_ipadic.py` が「％」と同じ品詞・文脈 ID・コストの語を読み付きで足す（units.csv、181 語）。`CoarsePos` も表層形で `NounSuffix` にする（`pos.rs` の `is_unit_symbol`。語を持たない辞書でも同じ値） |
