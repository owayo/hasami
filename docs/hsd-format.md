...
| ファイルサイズ | 配布する 3 辞書（ipadic / ipadic-neologd / ipadic-neologd-sudachi）のサイズ。Git LFS の転送量に直結する |
...
| トークン生成 | 全文字列の `Arc<str>` キャッシュを clone | 最良パスの語だけ復号。品詞・活用は表の `Arc` を clone |
...
| ipadic | **v4** | **17.3MB** | **0.01s / 4MB** | 13.7s / **23MB** | 12.0µs / 35.2µs |
| ipadic-neologd | v3 | 590.7MB | 1.33s / 579MB | 21.0s / 972MB | 10.6µs / 32.9µs |
| ipadic-neologd | **v4** | **211.9MB** | **0.01s / 4MB** | **15.9s / 195MB** | 12.6µs / 36.2µs |
| ipadic-neologd-sudachi | v3 | 666.8MB | 1.43s / 629MB | 34.3s / 1078MB | 58.6µs / 96.4µs |
| ipadic-neologd-sudachi | **v4** | **227.2MB** | **0.01s / 4MB** | **15.4s / 209MB** | **11.9µs / 33.7µs** |
...
- **温まった状態で同じ文を繰り返すと v4 は 10〜15% 遅い**。v3 はトークンの文字列を全部キャッシュから clone していたが、v4 は最良パスの語ごとに
  素性レコードを復号して文字列を確保する（1 トークンあたり確保が 2〜4 回増える）。コーパスのように辞書のページが冷えている・語が散らばる
  使い方では、キャッシュが無い分だけ v4 が速い。次に詰めるなら、トークン生成の確保を減らす（読みを 1 回の確保で `Arc<str>` にする、
  よく出る語の文字列を解析器ごとに小さくキャッシュする）
...
| char.def の範囲が重なるとき、MeCab は後の行が勝つが、hasami は開始位置が大きい範囲が勝つ（「々」は MeCab では SYMBOL、hasami では KANJI） | 未対応。hasami の方が漢字の繰り返し記号として自然なので残している |
| char.def の group=1 と length=N の組み合わせ。MeCab は「同じ種類の文字の並び全体」と「N 文字までの接頭辞」を候補にするが、hasami は並びを N 文字で打ち切った 1 つだけを作る | 未対応（未知語のカタカナ語が長いと割れる原因になりうる） |

## 8. 次に検討すること

- Viterbi の集約（4.5）。ノード数 × 前の語の数が支配的なので、辞書が大きいほど効く見込み
- 構築のピークメモリ。推奨辞書の repair は最大 RSS 約 3GB（全エントリを `Arc<str>` 10 個の `DictEntry` で持つため）。
  品詞・活用は共有しているが、表層形・読み・原形は 1 件ずつ確保している
- 未知語の文字分類（7 章の残り 2 件）。形式ではなく char_class の問題
