...
//!
//! どの処理も入力の長さに比例する時間で済む。入力はバイト単位で 1 度だけ走査し、分割に関わらない
//! 文字は復号せずに読み飛ばす。括弧の対応は 1 パスのスタックで取る。例外語の照合は文末記号を起点に、
//! 前へはトライを辿り、後ろは二分探索で調べる。前へ辿る照合が入力の 1 文字の上を通る回数は、1 語の
//! 中の文末記号の数（組み込みの表では最多 9 個、利用者の語は 16 個まで）で抑えられる。
//!
//! ```
...
pub use exceptions::{builtin_exceptions, extract_candidates};
...
impl Splitter {
    ...
    pub fn split(&self, text: &str) -> Vec<Sentence> {
        ...
        while let Some(&mark) = marks.get(i) {
            ...
            if brackets.get(b).is_some_and(|r| r.start <= mark.pos) {
                // 対応の取れた括弧の内側では分割しない
